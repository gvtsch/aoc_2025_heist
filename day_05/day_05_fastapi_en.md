And here’s another fundamental concept for our project: FastAPI.
...
### Local LLM
The classic... connecting to the local LLM.
...
Pydantic helps us with structured output and data validation.
...
1. **Input and validation**  
   The function gets a `request` object of type `ChatRequest`. It contains the message (`message`) and the agent type (`agent_type`). FastAPI and Pydantic automatically check if the data is correct.

2. **Manage conversation history**  
   There’s a dictionary called `conversations`. For each agent type, the previous conversation is stored there. If there’s no history for the agent yet, a new list is created.

3. **Save user message**  
   The current user message is added to the history as a dictionary (`role: user, content: ...`).

4. **Generate response**  
   The LLM gets all previous messages (including the system message) and generates a response.  
   The system message tells the model what role to take (e.g. "You are a planner agent"). That’s similar to a persona you might pass in the prompt. The history is passed as a list of messages.

5. **Save agent response**  
   The agent’s response is also added to the history (`role: assistant, content: ...`).

6. **Return response**  
   The function returns a `ChatResponse` object. It contains the agent type, the generated response, and the number of tokens used (useful for costs and limits).
...
1. **Parameter passing**  
   The agent type is passed as part of the URL (e.g. `/history/planner`). FastAPI extracts this value and passes it as a string to the function.

2. **Retrieve history**  
   The function looks up the appropriate agent in the `conversations` dictionary and returns the saved conversation as a list. If there’s no history, an empty list is returned.

3. **Structure response**  
   The response is a dictionary with three fields:
   - `"agent"`: The name of the agent
   - `"messages"`: The complete conversation history as a list of messages
   - `"count"`: The number of messages in the history
...
#### Root
This is also a GET endpoint. The difference here is that you’re directed to the base URL.
...
It’s just a little homepage for the API – here you get an overview of where to find the main endpoints and the docs. Handy for anyone discovering or testing the API.
