#!/bin/bash
# Day 16: Start All Services
# Starts OAuth, Memory Service, and all Tool Services in background

echo "🚀 Starting Day 16 Services..."
...
# Start Memory Service (Port 8005)
echo "🧠 Starting Memory Service (Port 8005)..."
python day_16/memory_service.py > /tmp/memory_service.log 2>&1 &
MEMORY_PID=$!
...
echo "   Memory Service:    http://localhost:8005 (PID: $MEMORY_PID)"
echo ""
echo "📋 Logs in /tmp/*_service.log"
echo "🛑 To stop: ./day_16/stop_services.sh"
echo ""
echo "🎯 Ready to run: python day_16/integrated_system.py"